...
// requestListCIDOffers is used at start-up to request a list of DHT Offers from a provider with a given provider id.
func requestListCIDOffers(reader *fcrp2pserver.FCRServerReader, writer *fcrp2pserver.FCRServerWriter, args ...interface{}) error {
	//cidMin, cidMax *cid.ContentID, providerID *nodeid.NodeID, settings settings.AppSettings
	// Get parameters
	if len(args) != 3 {
	...
	if ack.Sign(c.GatewayPrivateKey, c.GatewayPrivateKeyVersion) != nil {
		return errors.New("Error in signing the ack")
	}

	return writer.Write(ack, c.Settings.TCPInactivityTimeout)
}
