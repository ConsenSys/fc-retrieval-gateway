...
func HandleGatewayPingRequest(w rest.ResponseWriter, request *fcrmessages.FCRMessage) {
	// Get core structure
	...
	_, nonce, _, err := fcrmessages.DecodeGatewayPingRequest(request)
	if err != nil {
	...
	// Construct message
	response, err := fcrmessages.EncodeGatewayPingResponse(nonce, true)
	if err != nil {
	...
	if err != nil {
		s := "Internal error: Fail to write message."
		logging.Error(s + err.Error())
		rest.Error(w, s, http.StatusInternalServerError)
	}
}
